...
class User_Facebook(User):
    ...
    def getMyPosts(self, dateMin="", dateMax="", limit=100):
        ...
        while ("data" in r and len(r["data"]) > 0):
            ...
            if ("next" in r["paging"]):
                ...
                while (i < 100):
                    try:
                        r = requests.get(r["paging"]["next"], timeout=(10, 10)).json()
                        break
                    except:
                        i = i + 1
            else:
    ...
    def __str__(self):
        return "USER:[name=<" + str(self.name) + ">; id=<" + str(self.id) + ">; token=<" + str(
            self.token) + ">; Type=<" + str(self.type) + ">]"
    ...
    def getPostsFromGroup(self, id, dateMin="", dateMax="", limit=100):
        ...
        posts = requests.get(
            "https://graph.facebook.com/v2.5/" + id + "/feed?fields=id,caption,created_time,description,feed_targeting,from,icon,is_hidden,is_published,link,message,message_tags,name,object_id,parent_id,picture,place,privacy,properties,shares,source,status_type,story,targeting,to,type,updated_time,with_tags&access_token=" + self.token).json()
        lista = _Post_Facebook.Post_Facebook.createListFromData(posts, limit, dateMin, dateMax)
        return lista
    ...
    def _getAdcontracts(self):
        r = requests.get(
            "https://graph.facebook.com/v2.6/me/Adcontracts?fields=account_id,account_mgr_fbid,account_mgr_name,adops_person_name,advertiser_name,agency_name,campaign_name,created_by,created_date,io_number,io_type,last_updated_by,last_updated_date,max_end_date,mdc_fbid,min_start_date,salesrep_fbid,salesrep_name,status,subvertical,thirdparty_billed,thirdparty_password,thirdparty_uid,thirdparty_url,version,vertical&access_token=" + self.token).json()
        lista = list()
        while ("data" in r and len(r["data"]) > 0):
            for a in r["data"]:
    ...
    def getAdmined_Groups(self):
        r = requests.get(
            "https://graph.facebook.com/v2.6/me/Admined_Groups?fields=id,cover,description,email,icon,link,member_request_count,name,owner,parent,privacy,updated_time,cover_url&access_token=" + self.token).json()
        lista = list()
        while ("data" in r and len(r["data"]) > 0):
            for a in r["data"]:
    ...
    def getAlbums(self):
        r = requests.get(
            "https://graph.facebook.com/v2.6/me/Albums?fields=id,can_upload,count,cover_photo,created_time,description,event,from,link,location,name,place,privacy,type,updated_time&access_token=" + self.token).json()
        lista = list()
        while ("data" in r and len(r["data"]) > 0):
            for a in r["data"]:
    ...
    def getApprequests(self):
        r = requests.get(
            "https://graph.facebook.com/v2.6/me/Apprequests?fields=id,action_type,application,created_time,data,from,message,object,to&access_token=" + self.token).json()
        lista = list()
        while ("data" in r and len(r["data"]) > 0):
            for a in r["data"]:
    ...
    def getEvents(self):
        r = requests.get(
            "https://graph.facebook.com/v2.6/me/Events?fields=id,can_guests_invite,cover,description,end_time,guest_list_enabled,is_page_owned,is_viewer_admin,name,owner,parent_group,start_time,ticket_uri,timezone,updated_time&access_token=" + self.token).json()
        lista = list()
        while ("data" in r and len(r["data"]) > 0):
            for a in r["data"]:
    ...
    def getGroups(self):
        r = requests.get(
            "https://graph.facebook.com/v2.2/me/Groups?fields=id,cover,description,email,icon,link,member_request_count,name,owner,parent,privacy,updated_time,cover_url&access_token=" + self.token).json()
        lista = list()
        while ("data" in r and len(r["data"]) > 0):
            for a in r["data"]:
                lista.append(Group(a))
    ...
    def getLive_Videos(self):
        r = requests.get(
            "https://graph.facebook.com/v2.6/me/Live_Videos?fields=id,broadcast_start_time,creation_time,description,from,is_reference_only,live_views,permalink_url,seconds_left,status,title,total_views,video&access_token=" + self.token).json()
        lista = list()
        while ("data" in r and len(r["data"]) > 0):
            for a in r["data"]:
    ...
    def getPhotos(self):
        r = requests.get(
            "https://graph.facebook.com/v2.6/me/Photos?fields=id,album,backdated_time,backdated_time_granularity,can_delete,can_tag,created_time,from,height,icon,images,link,name,name_tags,page_story_id,picture,place,updated_time,width&access_token=" + self.token).json()
        lista = list()
        while ("data" in r and len(r["data"]) > 0):
            for a in r["data"]:
    ...
    def getPicture(self):
        r = requests.get(
            "https://graph.facebook.com/v2.6/me/Picture?fields=height,is_silhouette,url,width&redirect=0&access_token=" + self.token).json()
        print(r)
        return r["data"]["url"]
    ...
    def getTelevision(self):
        r = requests.get(
            "https://graph.facebook.com/v2.6/me/Television?fields=id,about,affiliation,app_id,app_links,artists_we_like,attire,awards,band_interests,band_members,best_page,bio,birthday,booking_agent,built,can_checkin,can_post,category,category_list,checkins,company_overview,contact_address,country_page_likes,cover,culinary_team,current_location,description,description_html,directed_by,display_subtext,emails,features,food_styles,founded,general_info,general_manager,genre,global_brand_page_name,global_brand_root_id,has_added_app,hometown,hours,influences,is_community_page,is_permanently_closed,is_published,is_unclaimed,is_verified,last_used_time,leadgen_tos_accepted,link,location,members,mission,mpg,name,network,new_like_count,offer_eligible,owner_business,parent_page,parking,payment_options,personal_info,personal_interests,pharma_safety_info,phone,place_type,plot_outline,press_contact,price_range,produced_by,products,public_transit,record_label,release_date,restaurant_services,restaurant_specialties,schedule,screenplay_by,season,single_line_address,starring,store_number,studio,talking_about_count,unread_message_count,unread_notif_count,unseen_message_count,username,voip_info,website,were_here_count,written_by&access_token=" + self.token).json()
        lista = list()
        while ("data" in r and len(r["data"]) > 0):
            for a in r["data"]:
                lista.append(Page(a))
    ...
    def getVideo_Broadcasts(self):
        r = requests.get(
            "https://graph.facebook.com/v2.6/me/Video_Broadcasts?fields=id,broadcast_start_time,creation_time,description,from,is_reference_only,live_views,permalink_url,seconds_left,status,title,total_views,video&access_token=" + self.token).json()
        lista = list()
        while ("data" in r and len(r["data"]) > 0):
            for a in r["data"]:
                lista.append(Live_Videos(a))
    ...
    def getVideos(self):
        r = requests.get(
            "https://graph.facebook.com/v2.6/me/Videos?fields=backdated_time,backdated_time_granularity,id,created_time,description,embed_html,format,from,icon,is_instagram_eligible,length,permalink_url,picture,place,privacy,source,status,updated_time&access_token=" + self.token).json()
        lista = list()
        while ("data" in r and len(r["data"]) > 0):
            for a in r["data"]:
    ...
    def postVideo(self, path,message=" "):
         random_string = _Settings.Utility.randomword(50)
         extension=os.path.splitext(path)[1]
         file= _Settings.copyfile(path,random_string+extension)
         url="https://graph-video.facebook.com/"+self.id+"/videos?access_token="+self.token
    ...
    def postPhoto(self, path,message=" "):

         random_string = _Settings.Utility.randomword(50)
         extension=os.path.splitext(path)[1]
         file= _Settings.copyfile(path,random_string+extension)
         url="https://graph.facebook.com/v2.6/"+self.id+"/photos?access_token="+str(self.token)
         files={'file':open(path,'rb')}
